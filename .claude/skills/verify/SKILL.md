...
3. Confirm JavaScript, TypeScript, and Python are enabled while Rust and Go remain disabled.
4. Switch between JavaScript, TypeScript, and Python and confirm both Monaco model language IDs follow the selection.
5. Type into the first Monaco editor and confirm Monaco's first model value changes.
6. Submit JavaScript, TypeScript, and Python refactor requests and confirm the second Monaco model updates from the input state and uses the selected language.
...
9. Return mismatched supported, upcoming, and unknown fence tags; confirm Python and `py` are treated as supported while Rust and Go remain upcoming, and confirm hints do not change the selected language.
